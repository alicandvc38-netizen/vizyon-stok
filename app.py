...
def index():
    ...
    bugunku_satis = conn.execute("SELECT COALESCE(SUM(toplam),0) FROM satislar WHERE tarih LIKE ?", (bugun + "%",)).fetchone()[0]
    bekleyen_sevkiyat = conn.execute("SELECT COUNT(*) FROM sevkiyat WHERE durum != 'Teslim Edildi'").fetchone()[0]
    toplam_borc = conn.execute("SELECT COALESCE(SUM(tutar),0) FROM cari_hareketleri WHERE tip='Borç'").fetchone()[0]
    toplam_alacak = conn.execute("SELECT COALESCE(SUM(tutar),0) FROM cari_hareketleri WHERE tip='Alacak'").fetchone()[0]
    conn.close()
